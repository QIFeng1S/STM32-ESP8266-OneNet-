...
/**
  ...
  */
uint8_t OLED_DisplayBuf[8][128]; // OLED显示缓冲区
...
#define I2C_SDA_GPIO_PIN     GPIO_PIN_8
...
{
    My_I2C_Start();
    MyI2C_SendByte(0x78); // 发送设备地址
    MyI2C_ReceiveAck();   // 接收应答
    MyI2C_SendByte(0x00); // 发送命令标志  写命令d/c = 0.co = 0
    MyI2C_ReceiveAck();   // 接收应答
...
void OLED_WriteData(uint8_t *data, uint8_t count)
{
    ...
    MyI2C_SendByte(0x78); // 发送设备地址
    MyI2C_ReceiveAck();   // 接收应答
    MyI2C_SendByte(0x40); // 发送数据标志  写数据 d/c = 1.co = 0
    MyI2C_ReceiveAck();   // 接收应答
    for(uint8_t i = 0 ; i < count; i++)
    {
        MyI2C_SendByte(data[i]); // 发送数据
        MyI2C_ReceiveAck();   // 接收应答
    }
    My_I2C_Stop();
}
...
#define SPI_RES_GPIO_PIN     GPIO_PIN_14

#define SPI_DC_GPIO_PORT     GPIOB
#define SPI_DC_GPIO_PIN      GPIO_PIN_15
...
/**
  ...
  */
void OLED_W_D0(uint8_t value)
{
	/*根据value的值，将D0置高电平或者低电平*/
	HAL_GPIO_WritePin(SPI_CLK_GPIO_PORT, SPI_CLK_GPIO_PIN, (GPIO_PinState)value);
}
...
/**
  ...
  */
void OLED_W_D1(uint8_t value)
{
	/*根据value的值，将D1置高电平或者低电平*/
	HAL_GPIO_WritePin(SPI_MOSI_GPIO_PORT, SPI_MOSI_GPIO_PIN, (GPIO_PinState)value);
}
...
/**
  ...
  */
void OLED_W_RES(uint8_t value)
{
	/*根据value的值，将RES置高电平或者低电平*/
	HAL_GPIO_WritePin(SPI_RES_GPIO_PORT, SPI_RES_GPIO_PIN, (GPIO_PinState)value);
}
...
/**
  ...
  */
void OLED_W_DC(uint8_t value)
{
	/*根据value的值，将DC置高电平或者低电平*/
	HAL_GPIO_WritePin(SPI_DC_GPIO_PORT, SPI_DC_GPIO_PIN, (GPIO_PinState)value);
}
...
/**
  ...
  */
void OLED_W_CS(uint8_t value)
{
	/*根据BitValue的值，将CS置高电平或者低电平*/
	HAL_GPIO_WritePin(SPI_CS_GPIO_PORT, SPI_CS_GPIO_PIN, (GPIO_PinState)value);
}
...
void OLED_SPI_GPIO_Init(void)
{
	...
	GPIO_InitStruct.Pin = SPI_RES_GPIO_PIN;
	HAL_GPIO_Init(SPI_RES_GPIO_PORT, &GPIO_InitStruct);
	GPIO_InitStruct.Pin = SPI_DC_GPIO_PIN;
...
void OLED_Init(void)
{
    ...
    OLED_WriteCommand(0xDB); // 设置VCOMH去耦电压
    OLED_WriteCommand(0x30); //
    OLED_WriteCommand(0xA4); // 全局显示开启
    OLED_WriteCommand(0xA6); // 设置正常/反相显示
...
void OLED_Clear(void)
{
    for (uint8_t page = 0; page < 8; page++)
    {
        ...
        {
            OLED_DisplayBuf[page][col] = 0x00; // 清除数据缓存区
        }
    }
}
...
{   
    if(FrontSize == OLED_6X8)
    {
        OLED_ShowImage(x, y, 6, 8, OLED_F6x8[Char - ' ']);
    }
    else if(FrontSize == OLED_8X16)
    {
